...
with dwf.DigitalDiscovery() as device:
    ...
    cs1.setup(enabled=True, state=False)

    time.sleep(0.5)

    # a6 = ??
    # a6.setup(enabled=True, state=True)
    # a7 = ??
    # a7.setup(enabled=True, state=True)
    # a9 = ??
    # a9.setup(enabled=True, state=False)
    ...
    d0.setup(enabled=True, state=True)
    d1.setup(enabled=True, state=False)
    d2.setup(enabled=True, state=True)
    d3.setup(enabled=True, state=False)
    d4.setup(enabled=True, state=True)
    d5.setup(enabled=True, state=False)
    d6.setup(enabled=True, state=True)
    d7.setup(enabled=True, state=False)
    ...
    print(b==[True, False, True, False, True, True, False, True, False]) 
